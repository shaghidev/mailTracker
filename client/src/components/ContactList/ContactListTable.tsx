...
interface Props {
  contacts: Contact[];
  onDelete: (email: string) => void; // delete preko emaila
}
...
const ContactTable: React.FC<Props> = ({ contacts, onDelete }) => {
  return (
    <table className="w-full text-left border-collapse">
      ...
      <tbody>
        {contacts.map(c => (
          <tr key={c.email} className="bg-[#0A0F12] text-[#FFFFFF] hover:bg-[#1F2937]">
            ...
            <td className="p-2">
              <button
                ...
                className="bg-[#EF4444] py-1 px-3 rounded hover:bg-[#DC2626]"
              >
                Delete
              </button>
            </td>
