...
const DashboardPage = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-[#080D10] p-6 text-[#FFFFFF]">
      <div className="w-full max-w-6xl flex justify-between items-center mb-6">
        <h1 className="text-4xl font-extrabold text-[#FFBD00]">Dashboard</h1>
        <button
          onClick={() => router.push('/dashboard/new')}
          className="bg-[#2979FF] text-white px-5 py-2 rounded-lg hover:bg-[#1E63D8] transition-colors"
        >
          + New Campaign
        </button>
      </div>
      ...
      {loadingCampaigns ? (
        <p className="text-[#A0AEC0]">Loading campaigns...</p>
      ) : campaigns.length === 0 ? (
        <p className="text-[#A0AEC0]">No campaigns found.</p>
      ) : (
